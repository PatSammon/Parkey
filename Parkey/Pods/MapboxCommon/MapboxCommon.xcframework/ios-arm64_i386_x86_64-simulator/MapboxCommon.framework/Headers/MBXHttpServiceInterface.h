...
/**
 * @brief Set maximum number of requests to a single host.
 *
...
/**
 * @brief Sends (or enqueues) a platform's native HTTP Request.
 *
...
/**
 * @brief Cancels a pending or running request (or download request).
...
/** @brief Returns true when the HTTP service supports the keepCompression flag for HTTP requests, false otherwise. */
- (BOOL)supportsKeepCompression;
/**
 * @brief Send the download request.
